...
<xsl:stylesheet
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//tei:text/tei:body/tei:div[@type='entry']">
            <xsl:variable name="date" as="xs:string" select="normalize-space(tei:head/tei:date/@when)"/>
            <xsl:result-document href="{concat('diary/',$date,'.html')}" method="xhtml" indent="yes">
                <html lang="de">
                    <head>
                        <meta charset="utf-8"/>
                        <title>
                    ...
                    <body class="mode-reading">
                        <header>
                            ...
                            <h1><xsl:value-of select="tei:head/tei:date"/></h1>
                        </header>
        ...
        <xsl:result-document href="diary/calendar.html" method="xhtml" indent="yes">
            <html lang="en">
                ...
                <body class="mode-reading">
                    ...
                    <main>
                        <p>This is a placeholder. In the next step we'll render a full month-by-month calendar.</p>
                        <ul>
                            <xsl:for-each select="//tei:div[@type='entry']">
                                <li>
                                    <a href="{concat(normalize-space(tei:head/tei:date/@when),'.html')}">
                                        <xsl:value-of select="tei:head/tei:date"/>
                                    </a>
                                </li>
                            </xsl:for-each>
    ...
    <xsl:template match="tei:add">
        <span class="add" data-place="{@place}"><xsl:apply-templates/></span>
    </xsl:template>
    
    <!-- Persons / Works / Places / Orgs -->
    <xsl:template match="tei:persName">
        <span class="person" data-ref="{@ref}"><xsl:apply-templates/></span>
    </xsl:template>
    ...
    <xsl:template match="tei:placeName">
        <span class="place" data-ref="{@ref}"><xsl:apply-templates/></span>
    </xsl:template>
    
    <xsl:template match="tei:orgName">
        <span class="org" data-ref="{@ref}"><xsl:apply-templates/></span>
    </xsl:template>
    ...
    <xsl:template match="tei:note[@place='left']">
        <span class="note-icon author" title="Note by Schönberg" data-note-id="{generate-id()}">🖊️</span>
    </xsl:template>
    
    <xsl:template match="tei:note[@type='comment']">
        <span class="note-icon editor" title="Editor's note" data-note-id="{generate-id()}">ℹ️</span>
    </xsl:template>
